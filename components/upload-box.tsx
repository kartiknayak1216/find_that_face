...
export default function UploadBox() {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      formData.append("user_id", "123") // Replace with actual user ID logic if needed
      formData.append("username", "kartik")
      ...
      const { url, token } = await uploadRes.json()

      // ✅ Step 2: Navigate to /results with URL & token
      const params = new URLSearchParams({ url, token }).toString()
